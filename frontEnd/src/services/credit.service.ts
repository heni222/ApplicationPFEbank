...
export class CreditService {
  ...
  loadInitialData(): void {
    ...
    // GET /api/clients
    this.http.get<Client[]>(`${this.API}/clients`).pipe(
      catchError(e => { console.error('clients:', e); return [[]]; })
    ).subscribe(c => this._clients.next(c as Client[]));

    // GET /api/applications
    this.http.get<CreditApplication[]>(`${this.API}/applications`).pipe(
      catchError(e => { console.error('applications:', e); return [[]]; })
    ).subscribe(a => {
    ...
    // GET /api/stats/kpis
    this.http.get<KPIs>(`${this.API}/stats/kpis`).pipe(
      catchError(e => { console.error('kpis:', e); return [this._kpis.value]; })
    ).subscribe(k => this._kpis.next(k as KPIs));

    // GET /api/stats/monthly
    this.http.get<MonthlyStat[]>(`${this.API}/stats/monthly`).pipe(
      catchError(e => { console.error('monthly:', e); return [[]]; })
  ...
  updateClient(id: string, dto: Partial<CreateClientDTO>): Observable<Client> {
    return this.http.put<Client>(`${this.API}/clients/${id}`, dto).pipe(
      tap(updated => {
        this._clients.next(this._clients.value.map(c => c._id === id ? updated : c));
      }),
      catchError(e => this.handleError(e))
    );
  }
  ...
  deleteClient(id: string): Observable<void> {
    return this.http.delete<void>(`${this.API}/clients/${id}`).pipe(
      tap(() => this._clients.next(this._clients.value.filter(c => c._id !== id))),
      catchError(e => this.handleError(e))
    );
  }
  ...
  createApplication(dto: CreateApplicationDTO): Observable<CreditApplication> {
    return this.http.post<CreditApplication>(`${this.API}/applications`, dto).pipe(
      tap(app => {
  ...
  fetchApplications(filters?: ApplicationFilters): Observable<CreditApplication[]> {
    ...
    return this.http.get<CreditApplication[]>(`${this.API}/applications`, { params }).pipe(
  ...
  getApplicationById(id: string): Observable<CreditApplication> {
    return this.http.get<CreditApplication>(`${this.API}/applications/${id}`).pipe(
      catchError(e => this.handleError(e))
    );
  }
  ...
  deleteApplication(id: string): Observable<void> {
    return this.http.delete<void>(`${this.API}/applications/${id}`).pipe(
      tap(() => {
        this._applications.next(this._applications.value.filter(a => a._id !== id));
        this.refreshKPIs();
      }),
      catchError(e => this.handleError(e))
    );
  ...
  uploadDocument(applicationId: string, file: File): Observable<CreditDocument> {
    const fd = new FormData();
    fd.append('document', file, file.name); // champ = 'document' (multer)

    return this.http.post<CreditDocument>(
      `${this.API}/applications/${applicationId}/documents`, fd
    ).pipe(
      tap(doc => {
        const apps = [...this._applications.value];
        const idx = apps.findIndex(a => a._id === applicationId);
        if (idx !== -1) {
          apps[idx] = { ...apps[idx], documents: [...apps[idx].documents, doc] };
  ...
  validateDocument(applicationId: string, documentId: string, validatedBy: string): Observable<void> {
      ...
      tap(() => {
        ...
        if (appIdx !== -1) {
          const docs = apps[appIdx].documents.map(d =>
            d._id === documentId ? { ...d, status: 'VALIDE' as DocumentStatus, validatedBy } : d
          );
  ...
  addComment(applicationId: string, content: string, userName: string): Observable<Comment> {
    return this.http.post<Comment>(
      `${this.API}/applications/${applicationId}/comments`,
      { content, userName }
    ).pipe(
      tap(comment => {
        const apps = [...this._applications.value];
        const idx = apps.findIndex(a => a._id === applicationId);
        if (idx !== -1) {
          apps[idx] = { ...apps[idx], comments: [...apps[idx].comments, comment] };
          this._applications.next(apps);
        }
      }),
      catchError(e => this.handleError(e))
  ...
  calculateMonthly(amount: number, duration: number, rate = 0.055): number {
    const r = rate / 12;
    if (r === 0) return Math.round(amount / duration);
    return Math.round(
  ...
  calculateDebtRatio(client: Client, newMonthly: number): number {
    const existingMonthly = client.existingLoans > 0 ? client.existingLoans / 240 : 0;
    return Math.round(
      ((client.monthlyCharges + existingMonthly + newMonthly) / client.revenue) * 100
    );
  }
